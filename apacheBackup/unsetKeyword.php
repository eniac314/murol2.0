...
if(getenv('REQUEST_METHOD') == 'POST') {
  ...
  $query = "DELETE FROM keywords WHERE contentId = ? AND keyword = ?";
  mysqli_stmt_prepare($stmt, $query);
  mysqli_stmt_bind_param($stmt,'ss', $php_data->contentId, $php_data->keyword);
  mysqli_stmt_execute($stmt);

  if (mysqli_stmt_affected_rows($stmt) == 0){
    logError("impossible d'effacer le mot clé");
    mysqli_close($db);
    exit();
  }
