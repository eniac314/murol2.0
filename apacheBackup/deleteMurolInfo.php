...
if(getenv('REQUEST_METHOD') == 'POST') {
  ...
  if(!isset($php_data->sessionId) || !isset($php_data->issue) || !isset($php_data->name)){
  ...
  $query = "DELETE FROM murolInfos WHERE issue = ?";
  mysqli_stmt_prepare($stmt, $query);
  mysqli_stmt_bind_param($stmt,'s', $issue);
  mysqli_stmt_execute($stmt);

  if (mysqli_stmt_affected_rows($stmt) == 0){
    logError("impossible d'effacer le murol info");
    mysqli_close($db);
    exit();
  }
